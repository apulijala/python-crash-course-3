class Addition():
    ...
    when people provide text instead of numbers. When you try to convert the input to an int, 
    ...
    def add_numbers(self):
        try: 
            ...
            print(f"{num1} and {num2} must be valid numbers")
    ...
    user can continue entering numbers even if they make a mistake and enter text instead of a number.
    """
    
    def add_numbers_while_loop(self):
        while True:
            try:
                ...
                print(f"{num1} and {num2} must be numbers")
            else:
                result = num1 + num2
...
of dogs in the second file. Write a program that tries to read these files 
and print the contents of the file to the screen. Wrap your code in a try-except block to 
catch the FileNotFound error, and print a friendly message if a file is missing. 
Move one of the files to a different location on your system, 
and make sure the code in the except block executes properly.
"""
class CatsAndDogs():
    def __init__(self, catsfile, dogsfile):
        self.catsfile = catsfile
        self.dogsfile = dogsfile
    
    
    def getCatsFile(self):
        try: 
            with open(self.catsfile, "r") as catsreader:
                catslines = catsreader.readlines()
                for cat in catslines: 
                    print(f"{cat.rstrip()}")
        except FileNotFoundError as fe: 
            pass # For silent failures
            # print(fe)
            
        
    # Get the cats and then the dogs
    def getDogsFile(self):
         print(f"Cats and Dogs")
         try: 
            with open(self.dogsfile, "r") as dogsreader:
                for dog in dogsreader.readlines():
                    print(f"{dog.rstrip()}")
         except FileNotFoundError as fe: 
             pass # For silent failures
             # print(fe)
...
class CommonWords():
    ...
    def word_count(self, word):
        with open(self.textfile, "r") as reader:
            content = reader.read()
